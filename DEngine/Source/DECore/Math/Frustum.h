...
private:
	...
	void Reconstruct(const float fFov, const float fRatio, const float fZNear, const float fZFar)
	{
		Vector3 corners[8]; // clockwise from bottom left
							// top
		corners[0] = Vector3(-fZNear * tanf(fFov / 2.0f) * fRatio, fZNear * tanf(fFov / 2.0f), fZNear);
		corners[1] = Vector3(-fZFar * tanf(fFov / 2.0f) * fRatio, fZFar * tanf(fFov / 2.0f), fZFar);
		corners[2] = Vector3(fZFar * tanf(fFov / 2.0f) * fRatio, fZFar * tanf(fFov / 2.0f), fZFar);
		corners[3] = Vector3(fZNear * tanf(fFov / 2.0f) * fRatio, fZNear * tanf(fFov / 2.0f), fZNear);
		// bottom
		corners[4] = Vector3(-fZNear * tanf(fFov / 2.0f) * fRatio, -fZNear * tanf(fFov / 2.0f), fZNear);
		corners[5] = Vector3(-fZFar * tanf(fFov / 2.0f) * fRatio, -fZFar * tanf(fFov / 2.0f), fZFar);
		corners[6] = Vector3(fZFar * tanf(fFov / 2.0f) * fRatio, -fZFar * tanf(fFov / 2.0f), fZFar);
		corners[7] = Vector3(fZNear * tanf(fFov / 2.0f) * fRatio, -fZNear * tanf(fFov / 2.0f), fZNear);
